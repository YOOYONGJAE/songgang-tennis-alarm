...
# (선택) Upstash Management API 자격증명. 데이터 조회용 REST 토큰과는 별개인
# '계정 관리용' 값이라, 요금제 한도·사용량(분모)을 보려면 이 둘이 필요하다.
# 없으면 /사용량 은 키 개수만 알려주고 자격증명 추가를 안내한다(봇은 정상 동작).
UPSTASH_EMAIL = os.environ.get("UPSTASH_EMAIL")
UPSTASH_API_KEY = os.environ.get("UPSTASH_API_KEY")
MGMT_BASE = "https://api.upstash.com/v2"
...
COMMAND_ALIASES = {
    ...
    "db": "db", "디비": "db", "사용량": "db", "용량": "db", "데이터베이스": "db",
...
# ---------------------------------------------------------------------------
# Upstash Management API (요금제 한도·사용량 조회)
#   Basic Auth(이메일 + 관리용 API 키)로 https://api.upstash.com/v2 를 부른다.
#   REST 토큰은 '데이터 조회', 이 자격증명은 '계정/요금제 조회'로 쓰임이 다르다.
# ---------------------------------------------------------------------------
def mgmt_enabled():
    return bool(UPSTASH_EMAIL and UPSTASH_API_KEY)
...
def _rest_host():
    """REST URL 에서 호스트만 뽑는다(예: beloved-stallion-58500.upstash.io)."""
    return UPSTASH_URL.split("://", 1)[-1].split("/")[0]


def resolve_db_id():
    """이 봇이 쓰는 DB 의 database_id 를 관리 API 로 찾는다.

    REST URL 호스트와 각 DB 의 endpoint(전체 호스트 또는 슬러그)를 맞춰본다.
    한 번 찾으면 Upstash(mgmt_db_id)에 캐시해 매번 목록을 조회하지 않는다.
    """
    cached = redis("GET", "mgmt_db_id")
    if cached:
        return cached
    host = _rest_host()
    for db in mgmt_get("/redis/databases"):
        ep = db.get("endpoint") or ""
        if ep and (ep == host or host.split(".")[0] == ep or host.startswith(ep)):
            redis("SET", "mgmt_db_id", db["database_id"])
            return db["database_id"]
    return None


def _human_bytes(n):
    """바이트 수를 사람이 읽기 쉬운 단위로. 예: 1258291 → '1.2 MB'."""
    n = float(n or 0)
...
def db_status_text():
    """데이터베이스 사용량을 '사용량 / 전체용량' 형식으로 만든다.

    관리 API 자격증명이 있으면 저장용량·명령 수를 한도(분모)까지 채우고,
    없으면 REST 토큰으로 읽히는 키 개수만 보여주며 자격증명 추가를 안내한다.
    """
    keys = redis("DBSIZE")
    ...
    if not mgmt_enabled():
        return (
            "💾 데이터베이스 (Upstash)\n"
            f"• 키 개수: {keys}\n\n"
            "사용량/한도까지 보려면 관리 API 자격증명(UPSTASH_EMAIL,\n"
            "UPSTASH_API_KEY)을 GitHub Secrets 에 추가해주세요."
        )

    try:
        db_id = resolve_db_id()
        if not db_id:
            return "⚠️ 관리 API 에서 이 봇의 DB 를 찾지 못했어요. 자격증명·권한을 확인해주세요."
        info = mgmt_get(f"/redis/database/{db_id}?credentials=hide")
        stats = mgmt_get(f"/redis/stats/{db_id}")
    except Exception as ex:
        return f"⚠️ 사용량 조회에 실패했어요. ({type(ex).__name__})"

    mem_used = stats.get("current_storage") or 0
    mem_total = info.get("db_memory_threshold") or 0
    cmd_month = stats.get("total_monthly_requests") or 0
    cmd_limit = info.get("db_request_limit") or 0
    cmd_today = stats.get("daily_net_commands") or 0

    lines = ["💾 데이터베이스 사용량 (Upstash)", f"• 요금제: {info.get('type', '?')}"]
    if mem_total:
        lines.append(
            f"• 저장용량: {_human_bytes(mem_used)} / {_human_bytes(mem_total)}"
            f" ({mem_used / mem_total * 100:.1f}%)"
        )
    else:
        lines.append(f"• 저장용량: {_human_bytes(mem_used)}")
    if cmd_limit:
        lines.append(
            f"• 이번 달 명령: {cmd_month:,} / {cmd_limit:,}"
            f" ({cmd_month / cmd_limit * 100:.1f}%)"
        )
    else:
        lines.append(f"• 이번 달 명령: {cmd_month:,}")
    lines.append(f"• 오늘 명령: {cmd_today:,}")
    lines.append(f"• 키 개수: {keys}")
    return "\n".join(lines)
