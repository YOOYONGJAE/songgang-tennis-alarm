...
**(선택) 데이터베이스 사용량(`/사용량`)까지 보려면** 아래 2개를 더 등록합니다.
없어도 봇은 정상 동작하며, 이 경우 `/사용량` 은 키 개수만 알려줍니다.

| 이름 | 값 |
|------|-----|
| `UPSTASH_EMAIL` | Upstash 계정 이메일 |
| `UPSTASH_API_KEY` | Upstash 관리 API 키 |

관리 API 키는 위 데이터 저장용 REST 토큰과는 **다른 값**입니다.
Upstash 콘솔 우측 상단 계정 메뉴 → **Management API → Create API Key** 에서 발급합니다.
...
| `/상태` | 현재 상태 전반 보기 |
| `/사용량` | 데이터베이스 저장·명령 사용량 보기 (관리 API 자격증명 필요) |
| `/on`, `/off` | 알림 켜고 끄기 |
| `/도움말` | 명령 목록 |
...
텔레그램 입력창에 `/` 만 치면 명령 목록이 자동완성 메뉴로 떠 외울 필요가 없습니다.
텔레그램은 명령 이름에 영문만 허용해, 메뉴에는 영문 별칭(`/help`, `/status`, `/db`,
`/on`, `/off`, `/period`, `/court`)으로 뜹니다. 위 표의 한글 명령(`/기간` 등)도 그대로 동작합니다.
메뉴는 봇이 한 번 실행되면 자동 등록됩니다(별도 설정 불필요).
